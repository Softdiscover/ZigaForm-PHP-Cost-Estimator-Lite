...
class model_record extends CI_Model
{
    ...
    {
        parent::__construct();
        $this->table = $this->db->dbprefix . "cest_uiform_form_records";
        $this->tbform = $this->db->dbprefix . "cest_uiform_form";
        $this->tbformtype = $this->db->dbprefix . "cest_uiform_fields_type";
        $this->tbformfields = $this->db->dbprefix . "cest_uiform_fields";
    }
    ...
    function getVarOptRecord($option,$form_id) {
        if(intval($form_id)>0){
            
            $sql = 'select ';
            $temp = array();
            $temp[] = "extractvalue(fbh_data_rec2_xml,'/params/child::" . $option . "') AS uifmoptvalue";
            $temp[] = "r.fbh_id";
            $temp[] = "r.created_date";
            $sql.=implode(',', $temp) . ' from %1$s r';
            $sql.=' join %2$s frm on frm.fmb_id=r.form_fmb_id
    ...
    function getNameField($id_record) {
        $query = sprintf('select f.fmf_uniqueid,f.fmf_id, coalesce(NULLIF(f.fmf_fieldname,""),CONCAT(t.fby_name,f.fmf_id)) as fieldname ,f.type_fby_id,f.fmf_data
        from %s f 
        join %s t on f.type_fby_id=t.fby_id 
        join %s fm on fm.fmb_id=f.form_fmb_id
        join %s rc on rc.form_fmb_id=fm.fmb_id
        where rc.fbh_id=%s', $this->tbformfields, $this->tbformtype, $this->tbform, $this->table, (int)$id_record);
        $query2 = $this->db->query($query);
        return $query2->result();
    }
    
    function getNameInvoiceField($id_field) {
        $query = sprintf('select f.fmf_uniqueid,f.fmf_id, coalesce(NULLIF(f.fmf_fieldname,""),CONCAT(t.fby_name,f.fmf_id)) as fieldname 
        from %s f 
        join %s t on f.type_fby_id=t.fby_id 
        join %s fm on fm.fmb_id=f.form_fmb_id
        join %s rc on rc.form_fmb_id=fm.fmb_id
        where rc.fbh_id=%s and t.fby_id in (8,9,10,11,16,18,39,40,41,42)', $this->tbformfields, $this->tbformtype, $this->tbform, $this->table, (int)$id_field);
        $query2 = $this->db->query($query);
        return $query2->result();
    }
