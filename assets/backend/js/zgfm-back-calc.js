      ...
      */
				...
				for (let index in tmp_calc_arr_len) {
					...
					if ($('#uifm_frm_calc_content' + tmp_calc_arr_len[index]['id']).length) {
						tmp_txt_value = $('#uifm_frm_calc_content' + tmp_calc_arr_len[index]['id'])
			...
			this.preview_genTabContent = function () {
				...
				$.each(tmp_variable, function (index, value2) {
					...
					tmp_tab_cont
						.find('.sfdc-tab-content div[id=zgfm-menu-calc-tab-' + value2['id'] + ']')
						.find('.uifm_frm_calc_tabtitle')
						.val(value2['tab_title']);
           ...
           return;
			...
			this.calc_delete_tab = function () {
				...
				//refresh on backend
				$('#zgfm-tab-calc-sourcecode-wrapper  a[data-zgfm-order=' + index + ']')
					.parent()
					.remove();
				...
				//got to last variable
				$('#zgfm-tab-calc-list-mathvars').find('.zgfm-tab-calc-mathvar-item').last().find('a').trigger('click');
			...
			this.calc_dev_cleanAll = function () {
				//refresh keys
				this.calc_dev_cleanAll_fixkeys();
			...
			this.calc_dev_cleanAll_fixkeys = function () {
				...
				for (var key in tmp_calc) {
					if (tmp_calc[key].hasOwnProperty('id') && tmp_calc[key]['id'] != '') {
						...
						rocketform.setUiData3('calculation', 'variables', parseInt(tmp_calc[key]['order']), {
							hash: tmp_calc[key]['hash'] || '',
			...
			this.calc_addNew_onlyPreview = function (optindex, numorder) {
				...
				tmp_li =
					'<li class="' +
					tmp_class +
					' sfdc-active"><a href="#zgfm-menu-calc-tab-' +
					optindex +
					'" data-zgfm-order="' +
					numorder +
					'"  data-zgfm-id="' +
					optindex +
					'" data-toggle="sfdc-tab">' +
					tmp_tab_title +
					'</a></li>';
			...
			this.calc_refreshvars_init = function () {
				try {
					...
					//hide others
					var tmp_boxs = ['#uifm_frm_calc_cmbo_field_var2_wrapper', '#uifm_frm_calc_cmbo_field_var3_wrapper', '#uifm_frm_calc_cmbo_addvar'];
			...
			this.calc_variables_addVarByButton = function () {
				//get index
				var tmp_cur_index = $('#zgfm-tab-calc-sourcecode-wrapper .zgfm-tab-calc-mathvar-item.sfdc-active a').attr('data-zgfm-id') || 0;
			...
			this.calc_variables_showusedvars = function (index) {
				...
				var tmp_var_container = $('#uifm_frm_calc_content' + index)
					.parent()
			...
			this.calculation_getDetailField = function (value) {
				...
				if ($('#' + tmp_field_detail['unique_id']).length) {
					...
					var tmp_step_num = $('#' + tmp_field_detail['unique_id'])
						.closest('.uiform-step-pane')
						.data('uifm-step');
