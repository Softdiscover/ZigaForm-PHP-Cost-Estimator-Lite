...
window.rocketfm = rocketfm=$.rocketfm = function(){
   ...
   arguments.callee.setAccounting = function(obj) {
    ...
    arguments.callee.validate_form = function (el_form) {
      ...
      arguments.callee.action_refreshevents = function () {
        ...
        /* check radio button option when clicked on payment box*/
            ...
            var form_id=el.parent().find('._rockfm_form_id').val();
                                        ...
                                        if(response){
                                           ...
                                           if(parseInt(arrJson.success)===1){
                                               ...
                                               /*reset all other recaptchas from other forms if exists*/
                                                try {
    ...
    arguments.callee.wizard_nextButton = function (el) {
              var el_form=$(el).closest('.rockfm-form');
                ...
                //validation
                rocketfm.wizard_nextButton_validate(obj_cur_form,res_val, 
                    function(data){
   ...
   arguments.callee.payment_checkSelectedRdo = function (objwrap) {
       ...
       if(objwrap.find(".uiform-pg-order-cont input[type='radio']:checked").length > 0){
                                objwrap.find('.uiform-pg-content')
                                                .popover('destroy');
                                               $return=true; 
                            }else {
                                objwrap.find('.uiform-pg-content')
                                                .popover({
                                                    ...
                                                    placement:'top',
                                                    trigger: 'manual',
                                                    //template: '<div class="popover popover_selpay uiform_popover_frontend uiform-wrap" role="tooltip"><div class="arrow"></div><h3 class="popover-title"></h3><div class="popover-content"></div></div>',
