		...
		window.rocketfm = rocketfm = $.rocketfm = function () {
			...
			arguments.callee.action_refreshevents = function () {
				...
				$('.uiform-pg-order-cont').on('click', function () {
					$(this).find('.uiform-pg-radio-btn').find('input').prop('checked', true);
				});
			};
				...
				if (response) {
					...
					if (parseInt(arrJson.success) === 1) {
						if (el.find('.uiform-sticky-sidebar-box').length) {
							el.find('.uiform-sticky-sidebar-box').data('uiform_stickybox').destroy();
						}
			...
			arguments.callee.payment_seeSummary = function (element) {
				...
				$.ajax({
					...
					success: function (response) {
						...
						main_container.find('.uiform_modal_general').find('.sfdc-modal-body').html(decodeURIComponent(arrJson.show_summary));
						main_container.find('.uiform_modal_general').find('.sfdc-modal-title').html(arrJson.show_summary_title);
					}
				});
			};
							...
							case 'value':
								try {
									...
									result =
										...
										result_tmp.getDate() +
										'/' +
										result_tmp.getFullYear() +
										' ' +
										result_tmp.getHours() +
										':' +
										result_tmp.getMinutes() +
										':' +
										result_tmp.getSeconds();
								} catch (err) {
									result = '';
								}
				...
				obj_form.find('.rockfm-costest-field:not(.rockfm-conditional-hidden)').each(function () {
					...
					switch (parseInt(uifm_fld_type)) {
						...
						case 16:
							...
							uifm_fld_value = $(this).find('.rockfm-input4-slider').bootstrapSlider('getValue');
							...
							if (parseFloat(uifm_fld_sub_total) > 0) {
								...
								tmp_uifm_summ_list_inner +=
									' <span class="uiform-sbox-summ-fld-price"><ul><li>' + uifm_price_symbol + zgfm_front_cost.format_money(obj_form, uifm_fld_sub_total) + uifm_price_code + '</ul></li></span>';
							}
						...
						case 18:
							...
							if (parseFloat(uifm_fld_sub_total) > 0) {
								...
								tmp_uifm_summ_list_inner +=
									' <span class="uiform-sbox-summ-fld-price"><ul><li>' + uifm_price_symbol + zgfm_front_cost.format_money(obj_form, uifm_fld_sub_total) + uifm_price_code + '</li></ul></span>';
							}
						...
						switch (parseInt(tmp_field.attr('data-typefield'))) {
							...
							case 9:
								...
								tmp_field_inp.on(tmp_action, function (e) {
									...
									if ($(this).closest('.rockfm-field').hasClass('rockfm-costest-field')) {
										zgfm_front_cost.costest_refresh(obj_form);
									}
